...
# モデル読み込み
model_x = load_model("model_x.keras")
model_y = load_model("model_y.keras")
...
async def diagnose(request: Request, image: UploadFile):
    ...
    vec = img_array.reshape(1, 256, 256, 3)

    # モデルで予測
    pred_x = model_x.predict(vec)[0][0]
    pred_y = model_y.predict(vec)[0][0]
